...
/**
 * Fetch hospital name from opdconfig table.
 */
export async function getHospitalName(config: ConnectionConfig): Promise<string> {
  try {
    const response = await executeSqlViaApi('SELECT hospitalname FROM opdconfig LIMIT 1', config);
    const row = response.data?.[0];
    return String(row?.['hospitalname'] ?? '');
  } catch (error) {
